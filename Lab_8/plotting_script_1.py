...
filename     = "IV-1N4148.txt"              #filename : This is name of the file containing the x and y values
Plot_Title   =  "IV-Graph"
x_axis_label = "Voltage"
y_axis_label = "log(Current)"
...
if __name__ == '__main__':
    ...
    y_array = np.log(np.array(y_readings))
    ...
    for column in range(0, number_of_curves_in_plot):
        plt.scatter(x_array_final , y_array_final[column],label = legend_list[column], marker="x", s=60, c="r")
        
    plt.plot([0.308, 0.308], np.log([8.3E-7, 2.6E-6]), c='b', label = "VD1")
    plt.plot([0.675, 0.675], np.log([8.3E-7, 0.00423]), c='g', label = "VD2")
    plt.plot([0.27, 0.308], np.log([2.6E-6, 2.6E-6]), c='b', label = "ln(ID1)")
    plt.plot([0.27, 0.675], np.log([0.00423, 0.00423]), c='g', label = "ln(ID2)")
    plt.plot([0.308, 0.675], np.log([2.6E-6, 0.00423]), label = "Line Fit")
